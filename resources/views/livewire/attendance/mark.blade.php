<div>
    ...
    <div class="relative overflow-hidden rounded-2xl brand-gradient text-white p-6 mb-6">
        ...
        <div class="relative flex items-center gap-3">
            ...
            <div>
                <h1 class="font-heading text-xl sm:text-2xl font-bold">Attendance</h1>
                <p class="text-sm text-white/80 mt-0.5">Mark daily attendance for a class section.</p>
            </div>
        </div>
    </div>

    <div class="flex flex-col sm:flex-row gap-3 mb-4">
        <select wire:model.live="sectionId" class="w-full sm:w-64 min-h-touch rounded-lg border border-slate-300 px-3 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/50 focus:outline-none">
    ...
    @if ($sectionId)
            ...
            <form wire:submit="save" class="space-y-3 pb-24 lg:pb-0">
                <div class="card divide-y divide-slate-100 overflow-hidden">
                    @foreach ($students as $student)
                        @php($current = $status[$student->id] ?? 'present')
        ...
        @else
            <p class="text-sm text-slate-500">No students in this section yet.</p>
        @endif
    @else
        <p class="text-sm text-slate-500">Select a section to start marking attendance.</p>
    @endif
</div>
